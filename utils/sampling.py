...
def cifar_iid(dataset, num_users):
    ...
    # Ensure the number of users is a multiple of the number of classes
    if num_users % num_classes != 0:
        raise ValueError("Number of users must be a multiple of 10 for CIFAR-10.")

    # Dict of data indices for each user
    dict_users = {i: np.array([], dtype='int64') for i in range(num_users)}

    # Indices for each class
    idxs_class = {i: [] for i in range(num_classes)}

    # Assign indices to each class
    for idx, (_, target) in enumerate(dataset):
        idxs_class[target].append(idx)

    # Assign indices of each class to a specific group of users
    users_per_class = num_users // num_classes
    for class_idx, idxs in idxs_class.items():
        np.random.shuffle(idxs)
        items_per_user = len(idxs) // users_per_class
        for user_idx in range(users_per_class):
            user_id = class_idx * users_per_class + user_idx
            start_idx = user_idx * items_per_user
            end_idx = (user_idx + 1) * items_per_user if user_idx != users_per_class - 1 else len(idxs)
            dict_users[user_id] = np.array(idxs[start_idx:end_idx])
